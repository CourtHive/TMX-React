...
export const LinkEntry = ({ validate=true, onClose, onCancel, initialValues }) => {
    ...
    return (
        <Dialog disableBackdropClick={false} open={isOpen} maxWidth={'md'} onClose={cancelAction}>
            <DialogTitle><div style={{minWidth: 400}}>{initialValues.title}</div></DialogTitle>
            <DialogContent>
                <TextField
                    autoFocus
                    fullWidth
            ...
            <DialogActions style={{marginBottom: '1em'}}>
                <Button onClick={cancelAction} color="secondary" variant="outlined"> {t('ccl')} </Button>
                <Button onClick={handleSubmit(onSubmit)} disabled={false} color="primary" variant="outlined"> {t('save')} </Button>
            </DialogActions>
        </Dialog>
    )
};
...
export function defineLink({validate, link, title, prompt, callback}) {
    ...
    let onClose = (link) => {
        cleanUp();
        if (callback && typeof callback === 'function') { callback(link); }
    }
