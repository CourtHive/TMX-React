...
export function groupGrid(params) {
  ...
  let rowHeight = (r) => (r ? calc_height : calc_height * 0.8);
  let cw = (p) => (width * p) / 100;

  for (let mc = 0; mc < bracketData.positionsCount; mc++) {
    columns.push({ attr: 'score', pct: 50 / bracketData.positionsCount, mc });
  }
  ...
  function mapColumns(x, row, columns) {
    return columns.map((column, i) => {
      ...
      if (!row && !i) {
        cell.group_name = o.bracket_name || `${o.labels.group} ${bracketData.bracketIndex + 1}`;
      }
