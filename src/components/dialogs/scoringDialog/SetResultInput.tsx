...
import {
  ...
  hasTiebreakWinner,
  hasWinner
} from 'components/dialogs/scoringDialog/utils';
...
const SetResultInput: React.FC<SetResultInputProps> = ({
  ...
  const handleOnChange = (event) => {
    ...
    // handle timed set
    if (timed) {
      handleOnChangedTimedSet(eventValue);
      return false;
    }
    ...
    const isBiggerThanMax = isTiebreak ? parseInt(eventValue) > tiebreakTo : parseInt(eventValue) > maxLowSetScore;
      ...
      .map((currentSet) => {
        if (set.setNumber === currentSet.setNumber) {
          ...
          if (isTiebreak || currentSet.isTiebreakSet) {
            ...
            const editModeTiebreak = !!set?.tiebreak?.side1 && !!set?.tiebreak?.side2;
            ...
            const [side1ValueTiebreak, side2ValueTiebreak] = drawEngine.getTiebreakComplement({
              isSide1,
              lowValue: eventValue,
              tiebreakTo,
              tiebreakNoAd
          ...
          const [side1Value, side2Value] =
            set.winner !== SetWinnerEnum.NONE
              ? [
                  isSide1 ? parseInt(eventValue) : parseInt(set.side1),
                  !isSide1 ? parseInt(eventValue) : parseInt(set.side2)
                ]
