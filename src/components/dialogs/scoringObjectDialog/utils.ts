...
export const getValue = (
  ...
  const side1Result = isSide1
    ...
    : valueAsNumber + 1 < to
    ? to
    : (tiebreakNoAd && valueAsNumber === to) ||
      (!tiebreakNoAd && valueAsNumber === to - 2) ||
      // (!isTiebreakSet && tiebreakAt && tiebreakAt < to)
...
export const defaultChangeCategory = (
  ...
  const status = {
    ...
    matchUpStatusCodeDisplay: firstSubcategory.matchUpStatusCodeDisplay,
    matchUpStatusCode: firstSubcategory.matchUpStatusCode
  };
  ...
  const lastIsTiebreakOrTiebreakSet = lastSet?.tiebreak || lastSet.isTiebreakSet;
