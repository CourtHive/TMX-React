...
import { TeamEdit } from 'components/tables/teamsTable/TeamEdit/teamEdit';
...
export const TeamsTable = () => {
  ...
  const hiddenColumns = useSelector((state: any) => state.tmx.hiddenColumns.teams) || [];
  ...
  const handleOnClickEdit = () => {
    ...
    const participant = tournamentParticipants.find((p) => p.participantId === participantId);
    if (participant) {
      setTeamData({ teamParticipant: participant });
    }
  };
  ...
  const deleteSelectedParticipants = () => {
    ...
    dispatch({
      type: 'alert dialog',
      payload: {
        title: `${t('delete')} ${t('tmz')}`,
        content: `${t('delete')} selected teams?`,
        cancel: true,
        okTitle: 'Delete',
        ok: doIt
      }
  ...
  const checkedTeamIds = (tableData?.filter((team) => team.checked) || []).map((p) => p.participantId);
  ...
  const columns = [
    ...
    {
      key: 'code',
      getTitle: () => ({ node: t('Code'), className: classes.countColumn }),
      getValue: (row) => ({ node: row.code }),
      hidden: () => isHidden('code')
    },
    {
      key: 'members',
  ...
  const setColumnHiddenState = ({ key }) => {
    dispatch({
      type: 'hide column',
      payload: { table: 'teams', field: key, hidden: !isHidden(key) }
    });
  };
  ...
  const updateTeam = (updatedTeamParticipant) => {
    closeTeamDrawer();
    dispatch({
      type: 'tournamentEngine',
      payload: {
        methods: [
          {
            ...
            params: { participant: updatedTeamParticipant }
          }
        ]
      }
    });
  ...
  if (!dataForTable.length && !filterValue) {
    return (
      <>
        <Drawer anchor={'right'} open={Boolean(teamData)} onClose={closeTeamDrawer}>
          <TeamEdit {...teamData} updateTeam={updateTeam} cancel={closeTeamDrawer} />
        </Drawer>
        <NoticePaper className={'info'} style={{ marginTop: '1em' }}>
          <Grid container spacing={2} direction="row" justify="flex-start">
            <Grid item>Prompt to add team</Grid>
            <Grid item style={{ flexGrow: 1 }}>
              <Grid container direction="row" justify="flex-end">
                <AddGroupingButton onClick={addParticipant} label={t('Add Team')} />
              </Grid>
            </Grid>
          </Grid>
        </NoticePaper>
