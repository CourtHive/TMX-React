...
export function teamOptionNames({ teams, designator, drawDefinition }) {
  let upperName = (opponent) =>
    (opponent.name && opponent.name.toUpperCase()) || (opponent.last_name && opponent.last_name.toUpperCase()) || '';
...
export function opponentName({ opponent, designator, length_threshold }) {
  ...
  if (opponent.name) {
    ...
    let first_first_name =
      opponent.first_name && opponent.first_name.split(' ').length > 1
        ? `, ${opponent.first_name.split(' ')[0]}`
        : first_name;
    let last_last_name =
      opponent.last_name && opponent.last_name.trim().split(' ').length > 1
        ? opponent.last_name.trim().split(' ').reverse()[0]
        : opponent.last_name;
