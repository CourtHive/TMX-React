export const eventManager = function() {
   ...
   em.reset = () => { registeredFunctions = {}; return em; }
   ...
   function processEvnt({ evt, evnt, stopPropagation=true }) {
      ...
      var mouse = { x: evt.clientX, y: evt.clientY, pageX: evt.pageX, pageY: evt.pageY };
      ...
      if (cls_matches.length) {
         ...
         last_tap = this_tap;
         cls_matches.forEach(cls => { callFunction({ cls, evt, evnt, mouse }) });
      }
   }
   ...
   document.addEventListener('touchend', function() { touchleave(); }, false);
