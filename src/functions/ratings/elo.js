export const eloRatings = function() {
   ...
   elo.expect = (player_rating, opponent_rating) => { return 1 / (1 + Math.pow(10, ((opponent_rating - player_rating) / elo.nSpread))); }
   ...
   elo.newRating = ({
      ...
      let winner_converted_rating = convertRange({ value: w_new_rating, existing_range: elo.range, target_range: rating_range });
      let loser_converted_rating =  convertRange({ value: l_new_rating,  existing_range: elo.range, target_range: rating_range });
      ...
      if ((winner_rating > loser_rating && pct_diff  > elo.diffThreshold) || (winner_new_rating < 0 || loser_new_rating < 0)) {
   ...
   function inRange(range, value) { return value >= range[0] && value <= range[1]; }

   // see footnote #3 here:
   // http://fivethirtyeight.com/features/serena-williams-and-the-difference-between-all-time-great-and-greatest-of-all-time/
   function k538(matchUps) { return 250 / Math.pow(matchUps + 5, .4); }
   ...
   function convertRange({ value, existing_range, target_range }) { 
      return ( value - existing_range[0] ) * ( target_range[1] - target_range[0] ) / ( existing_range[1] - existing_range[0] ) + target_range[0];
   }
   return elo;
