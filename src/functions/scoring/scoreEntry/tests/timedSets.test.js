...
it('can enter single timed set', () => {
  ...
  ({ matchUp } = calcScore({ value: 2, matchUp, matchUpFormat }));
  expect(matchUp?.score.trim()).toEqual('2');
  expect(matchUp?.sets).toEqual([{ setNumber: 1, side1Score: 2, side2Score: undefined, winningSide: undefined }]);
...
it('does not consider tied score to be complete set', () => {
  ...
  ({ matchUp } = enterValues({ values, matchUp }));
  expect(matchUp?.score.trim()).toEqual('3-323 3-2');
