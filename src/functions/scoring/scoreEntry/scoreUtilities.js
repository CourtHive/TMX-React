...
export function removeFromScore({ analysis, matchUp, lowSide }) {
  ...
  const { isTiebreakEntry: isMatchTiebreak } = testTiebreakEntry({ score, brackets: MATCH_TIEBREAK_BRACKETS });
  ...
  if (index >= 0) {
    ...
    if (isMatchTiebreak && openMatchTiebreak) {
      ...
      if (side2TiebreakScore) {
        ...
        if (matchTiebreakScores[highIndex] < tiebreakTo) matchTiebreakScores[highIndex] = tiebreakTo;
...
export function checkValidMatchTiebreak({ score }) {
  ...
  const lastOpenBracketIndex = Math.max(...indices(open, splitScore));
  const lastCloseBracketIndex = Math.max(...indices(close, splitScore));
  const lastJoinerIndex = Math.max(...indices(MATCH_TIEBREAK_JOINER, splitScore));
  const isValid =
...
export function getMatchUpWinner({ analysis, matchUp }) {
  ...
  const sideScores = matchUp.sets?.reduce(
    (scores, set) => {
  ...
  if (WINNING_STATUSES.includes(matchUp.matchUpStatus) && matchUp.winningSide) {
    matchUpWinningSide = matchUp.winningSide;
  }
