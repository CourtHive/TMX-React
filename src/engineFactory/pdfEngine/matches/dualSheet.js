...
export function dualSheet({ tournament={}, data, adhoc_matches, logo, selectedEvent, event }) {
  ...
  let matches = (adhoc_matches && adhoc_matches.length && adhoc_matches) || tieMatchUps(evt, data.dual_match.matchUp.muid);
...
function dualMatchBox(matchUp) {
  ...
  let existing = matchUp.matchUp.score ? convertStringScore({
  ...
  table: {
    ...
    body: [
           matchHeader(matchUp),
           teamScoreLine(matchUp, scores, 0),
           teamScoreLine(matchUp, scores, 1)
    ]
  },
  layout: {
    paddingLeft: () => 1,
