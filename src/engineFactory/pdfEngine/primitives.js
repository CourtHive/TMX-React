...
export function lastName(p) {
  return p.otherName || p.last_name;
}
export function fullName(p) {
  const fullName = !p ? '' : `${p.last_name.toUpperCase()}, ${normalizeName(p.first_name, false)}`;
  return (p && p.otherName) || fullName;
}
export function potentialBlock(p) {
  return lastName(p)
    ? normalizeName(lastName(p), false).toUpperCase()
    : p.qualifier
    ? i18n.t('qualifier')
    : i18n.t('unk');
}
