...
export function scheduleCell(matchUp, lines = false) {
  ...
  var first_team =
    matchUp.team_players && matchUp.team_players[0]
      ? teamName(matchUp, matchUp.team_players[0])
      : unknownBlock(matchUp, 0);
  var second_team =
    matchUp.team_players && matchUp.team_players[1]
      ? teamName(matchUp, matchUp.team_players[1])
      : unknownBlock(matchUp, 1);
  ...
  if (matchUp.event && matchUp.event.custom_category)
    display.round = `${matchUp.event.custom_category} ${matchUp.round_name || ''}`;
  ...
  var cell = {
    ...
    table: {
      ...
      body: [
        ...
        [
          {
            text: display.first_team || x,
            style: 'teamName',
            margin: [0, 0, 0, 0],
            bold: display.bold1,
            color: display.color1,
            italics: display.italics1
          }
        ],
        [{ text: display.vs || x, style: 'centeredText', margin: [0, 0, 0, 0] }],
        [
          {
            text: display.second_team || x,
            style: 'teamName',
            margin: [0, 0, 0, 0],
            bold: display.bold2,
            color: display.color2,
            italics: display.italics2
          }
        ],
        [{ text: display.spacer || x, style: 'centeredText', margin: [0, 0, 0, 0] }],
        [
          {
            text: display.scoreline || x,
            style: 'centeredText',
            margin: [0, 0, 0, 0],
            bold: display.boldscore,
            color: display.colorscore
          }
        ]
      ]
    },
    layout: {
      ...
      hLineWidth: function (i, node) {
        return lines && (i === 0 || i === node.table.body.length) ? 1 : 0;
      },
      vLineWidth: function (i, node) {
        return lines && (i === 0 || i === node.table.widths.length) ? 1 : 0;
      }
    }
  };
  return cell;

  function playerItalics(pindex) {
    return unknowns.indexOf(pindex) >= 0;
  }
...
function teamName(matchUp, team) {
  ...
  if (!p) {
    ...
    let team_codes = team
      .reduce(
        (p, c) =>
          !matchUp.players[c] || p.indexOf(matchUp.players[c].team_code) >= 0
            ? p
            : p.concat(matchUp.players[c].team_code),
        []
      )
      .filter((f) => f)
      .join('/');
