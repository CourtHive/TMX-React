...
export function scheduleDocDefinition({ tournament, day, courts, matches, landscape, logo }) {
  ...
  courts.forEach((court, i) => {
     let court_header = env.schedule.court_identifiers ? (court.name || '') : (court.name && court.name.split(' ')[0]) || '';
     column_headers[i] = court_header;
     column_court_luids[i] = `${court.luid}${court.name}`;
  });

  let rows = row_matches
     .map((row, i) => column_court_luids.map(cl => row_matches[i].reduce((p, m) => `${m.schedule.luid}${m.schedule.court}` === cl ? m : p, {})));
